...
export const fetchNewReleases = createAsyncThunk('newReleases/fetchNewReleases', async () => {
    ...
    for (let page = 1; page < 67; page++) {
        ...
        results.map((result) => {
            if (moment(result.release_date).isBetween(lastWeek, tommorrow) && result.backdrop_path != null && result.poster_path != null && arr.length < 30) {
                arr.push(result)
            }
        })
        if (arr.length === 30) {
            ...
            arr.reduce((filter, current) => {
                var duplicate = filter.find(item => item.title === current.title);
                if (!duplicate) {
